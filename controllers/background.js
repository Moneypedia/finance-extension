...
function initialize_indexedDB() {
  ...
  dbRequest.onupgradeneeded = function (event) {
    ...
    objectStore.createIndex("creation_date_time", "creation_date_time", {
      unique: false,
    });
    objectStore.createIndex("last_test_date_time", "last_test_date_time", {
      unique: false,
    });
...
function insert_record(request) {
  ...
  dbRequest.onsuccess = function (event) {
    var db = event.target.result;
    var fcObjectStore = db
      .transaction("flashcard", "readwrite")
      .objectStore("flashcard");
    ...
    getRequest.onsuccess = function (event) {
      ...
      if (data == undefined) {
        ...
        var addRequest = fcObjectStore.add({
          vocab: request.vocab,
...
function send_msg(dataJSON, pageName) {
  chrome.tabs.query(
    { url: "chrome-extension://*/views/" + pageName + ".html" },
    function (tabs) {
  ...
  dbRequest.onsuccess = function (event) {
    ...
    index.get(vocab).onsuccess = function (event) {
      ...
      let dataJSON = {
        ...
        count: result.count,
        creationTime: result.creation_date_time,
        lastTestTime: result.last_test_date_time,
